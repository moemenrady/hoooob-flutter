...
import 'package:hoooob_app/features/location/domain/models/place_details_model.dart';
import 'package:hoooob_app/features/location/domain/models/prediction_model.dart';
import 'package:hoooob_app/features/location/domain/models/user_search_tripe_model.dart';
import 'package:hoooob_app/features/location/domain/models/zone_response.dart';
...
class LocationController extends GetxController implements GetxService {
  ...
  LocationController(
      {required this.locationServiceInterface, required this.stationService});
  ...
  List<int> addEntityList = [];
  int addEntity = 0;
  List<TripeData> allUserSearchTripeList = [];
  List<Map<String, dynamic>> restStopsList = [];
  ...
  final TextEditingController destinationLocationController =
      TextEditingController();
  late List<TextEditingController> restStopController;
  ...
  void resetPickData(LocationType type) {
    _pickAddress = '';
    _pickPosition = Position(
        longitude: 0,
        latitude: 0,
  ...
  Future userSearchTrip() async {
    print('111111111111111111111111111111');
    _isLoading = true;
    update();
    print(getPlaceWithLanLng(lat: 30.0444, lng: 31.2357).toString());
    try {
      Response response =
          await locationServiceInterface.searchUserTripe(UserSearchTripeModel(
        pickupLat: 30.0444,
        pickupLng: 31.2357,
        dropOffLat: 30.0606,
        dropOffLng: 31.2193,
      ...
      if (response.statusCode == 200) {
        var data = AllUserSearchTripeModel.fromJson(response.body);
        allUserSearchTripeList = data.data;

        _isLoading = false;
        update();
      }
    } on Response catch (failure) {
      showCustomSnackBar(
          failure.body['errors'][0]['message'] ?? failure.bodyString);
    }
  }
              ...
              : type == LocationType.to
                  ...
                  : type == LocationType.extraOne
                      ...
                      : type == LocationType.extraTwo
                          ? extraRouteTwoController.text
                          : type == LocationType.restStop
                              ? restStopController.last.text
                              : _position = Position(
  ...
  Future<void> saveAddressAndNavigate(
    ...
    if (type == LocationType.accessLocation) {
      ...
      } else if (type == LocationType.restStop) {
        restStopController.last.text = address.address!;
        stopRestAddress = address;
        restStopsList.add({
          'name': restStopController.last.text,
        ...
        if (fromSearch) {
          if (type == LocationType.from) {
            ...
            stopRestAddress = Address(
                ...
                longitude: latLng.longitude,
                address: address);
            restStopController.last.text = address;
          }
        }
  ...
  void setRestStop(LocationType type) {
    // restStopController.last.text = ;
  }
  ...
  void setAddAddressData(LocationType type) {
    if (type == LocationType.from) {
      ...
      print(
          '_updateAddAddressData=========================>$_updateAddAddressData');
    }
  ...
  // Breakpoints management methods
  void addBreakpoint() {
    _breakpoints.add({
      'address': null,
      'latitude': null,
  ...
  // Initialize static breakpoints for testing
  ...
  List<StationPointModel> searchResults = [];
  List<StationPointModel> nearestPoints = [];

  StationPointModel? selectedPoint;
  Future<List<StationPointModel>> searchStartPoints(String query) async {
    if (query.isEmpty) {
      searchResults = [];
      update();
      return [];
    ...
    try {
      final results = await stationService.searchStations(query: query);

      searchResults = results.take(4).toList();
...
// أقرب النقاط
  Future<List<StationPointModel>> getNearestStartPoints(
      LatLng cameraPosition) async {
    try {
      // جلب البيانات من السيرفس بالنسبة لإحداثيات الكاميرا
      final results =
          await stationService.searchStations(currentPosition: cameraPosition);

      // ترتيب حسب المسافة محليًا (لو السيرفر ما رجع المسافة)
      results.sort((a, b) {
        double distanceA = _calculateDistance(
            cameraPosition.latitude, cameraPosition.longitude, a.lat, a.lng);
        double distanceB = _calculateDistance(
            cameraPosition.latitude, cameraPosition.longitude, b.lat, b.lng);
        return distanceA.compareTo(distanceB);
      });

      nearestPoints = results.take(3).toList();
      update();

      return nearestPoints;
    } catch (e) {
      if (kDebugMode) print('Error fetching nearest stations: $e');
      nearestPoints = [];
      update();
      return [];
    }
  ...
  void selectStartPoint(StationPointModel point) {
    selectedPoint = point;
    ...
    _pickAddress = point.name;

    _buttonDisabled = false;
    update();
  }

  Future moveCameraToPoint(StationPointModel point) async {
  ...
  void onSearchResultTap(StationPointModel point) {
    selectStartPoint(point);
    moveCameraToPoint(point);
  }

  final List<StationPointModel> mockStartPoints = [
    StationPointModel(
      id: 1,
      name: "محطة الرمل",
      address: "محطة الرمل - الاسكندرية",
      lat: 31.2001,
      lng: 29.9187,
    ),
    StationPointModel(
