...
Future<void> withdrawRequest({required String withdrawMethod,required double amount }) async {


 isLoading = true;
    update();
    Response response = await paymentServiceInterface.withdrawRequest(withdrawMethod:withdrawMethod, amount: amount);
    if (response.statusCode == 200 ) {

      showCustomSnackBar('withdrawRequest Successful', isError: false);
